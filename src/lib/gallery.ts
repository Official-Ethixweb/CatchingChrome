...
/**
 * Optimized derivatives + intrinsic sizes, generated by
 * `node scripts/optimize-gallery.mjs` from the originals in public/Gallery.
 *
 * The originals are 2-3 MB camera files; the mosaic paints them at ~330 px
 * wide, so serving them raw meant downloading roughly 30x more pixels than
 * could ever be shown. Tiles now load a 720 px WebP (~66 KB) and the lightbox
 * fetches the 1600 px version only when a photo is actually opened.
 *
 * Sizes come from the same manifest so the layout is known before any image
 * loads — otherwise every row would reflow as bytes arrive.
 */
type Derivative = {
  thumb: string
  full: string
...
const DERIVATIVES = MANIFEST as Record<string, Derivative>

const keyOf = (src: string) => src.replace(G, '')

/** Small WebP for grid tiles. Falls back to the original if not yet generated. */
export function thumbOf(src: string): string {
  return DERIVATIVES[keyOf(src)]?.thumb ?? src
}

/** Larger WebP for the lightbox. */
export function fullOf(src: string): string {
  return DERIVATIVES[keyOf(src)]?.full ?? src
}

/** Intrinsic size of the rendered image, for width/height attributes. */
export function sizeOf(src: string): { width: number; height: number } | null {
  const d = DERIVATIVES[keyOf(src)]
...
export const MEDIA: MediaItem[] = [
  ...
  },
  {
    ...
    caption: 'Filleted, cleaned and packed before you leave',
    feature: true,
  },
  {
